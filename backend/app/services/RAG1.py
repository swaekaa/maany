...
If the query is related to a topic in the syllabus and the context provides partial or incomplete information, use what is available in the context and supplement it with accurate, additional knowledge on your own to provide an in-depth explanation.
If the query is not related to the context at all, reply like an academic assistant normally without referring to the context.
"ml" refers to Malayalam the langauage.
...
"""
    ...
    # ✅ Wrap sources as dictionaries
    sources = [{"source": meta.get("source", "N/A")} for meta in rag_output.get("metadata", [])]
